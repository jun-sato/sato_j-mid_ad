...
def main(datadir,organ,weight_path,outputdir,segtype,backbone,seed,amp=True):
    ...
    images_test = np.array([im for im in images_test if im!=None])
    overall_cutoff = []
    ...
    for n,(train_idxs, test_idxs) in enumerate(cv.split(images, labels, groups)):
        ...
        cutoff = 0.4#calc_metrics(y_pred,y,organ,dataset='validation',fold=str(n))
        ...
        with torch.no_grad():
            ...
            for test_data in test_loader:
                ...
                outputs = outputs[:,-1].view(test_images.size(0),input_size[2]//2) #最後の列がnofinding
                y_pred = torch.cat([y_pred, outputs], dim=0)
    ...
    overall_cutoff = 0.5#np.mean(overall_cutoff)
    # total_y_pred_test = total_y_pred_test/5
    test_group['pred'] = total_y_pred_test.cpu().numpy()
    test_group['label'] = total_y_test.cpu().numpy()
    ...
    print(total_y_pred_test,total_y_pred_test.max(),total_y_pred_test.min(),total_y_test)
    ...
    print(pred_df.shape,test_df.shape)
    groups = test_df['患者ID']
    columns = ['file','prediction','final_prediction','target','所見']
    pred_df.merge(test_df,on='file',how='left').to_csv(f'../result_eval/{organ}_with_finding_{str(seed)}.csv',index=False)
