...
def main(organ,num_epochs,num_classes,datadir,batch_size,save_model_name,backbone,segtype,seed=0,p_mixup=0,amp=True,use_buffer=True,metric_learning=False):
    ...
    total_labels = data_df['abnormal'].astype(float).values 
    g = torch.Generator()
    g.manual_seed(seed)
    ...
    for n,(train_idxs, test_idxs) in enumerate(cv.split(total_images, total_labels, groups)):
        ...
        #persistent_cache = pathlib.Path("/sqfs/work/K22A11/u6b588/jmid/", "persistent_cache")
        #persistent_cache.mkdir(parents=True, exist_ok=True)
        #labels_train = [np.array([label]*32) for label in labels_train]
        #train_ds = ImageDataset(image_files=images_train, labels=labels_train, transform=None,reader =monai.data.NumpyReader)
        train_ds = CLSDataset(filenames=images_train,labels=labels_train,transform=None)
        ...
        #model = monai.networks.nets.HighResNet(spatial_dims=3, in_channels=1, out_channels=num_classes).to(device)
        #model = monai.networks.nets.SEResNet152(spatial_dims=3,in_channels=1,num_classes=num_classes).to(device)
        model = TimmModel(backbone, pretrained=True).to(device)
            ...
            for batch_data in src:
                ...
                if amp and scaler is not None:
                    with torch.cuda.amp.autocast():
                        ...
                        loss = loss_function_bce(outputs, labels) #+ 0.02*loss_function_bce(outputs[:,1:],labels[:,1:]) 
                        if do_mixup:
