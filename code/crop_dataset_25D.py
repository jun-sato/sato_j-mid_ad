...
def focus_organs(organ_index:list,mask_path):
    ...
    '''
    try:
        default_img_path = image_dir + os.path.basename(mask_path).split('.')[0] + '.nii.gz'
        default_mask_path = mask_dir + os.path.basename(mask_path).split('.')[0][:-5] + '.nii.gz'
        ...
        scale = ScaleIntensityRange(
                    ...
                    clip=True,)
        resize = Resize(spatial_size=(64,384,384))
        ...
        default_array[~dilated_array] = -1000
        #print(default_array.shape)
        default_array = scale(default_array[np.newaxis])
        default_array = resize(default_array).detach().numpy()
        #print(default_array.shape)

        new_array  = np.concatenate([default_array[:,0:1,:,:],default_array[:,0:1,:,:],default_array[:,0:3,:,:]],axis=1)
        #print(new_array.shape)
        for n in np.arange(1,31,1):
            i = 2*n
            tmp = default_array[:,i-2:i+3,:,:]
            new_array = np.concatenate([new_array,tmp],axis=0)
        new_array = np.concatenate([new_array,default_array[:,59:,:,:]],axis=0)
        print(new_array.shape)

        #default_mask =  dilated_array
        dilated_array = dilated_array.astype(np.uint8)
        ...
        ## delated_arrayを保存
        #mask_preprocessed = sitk.GetImageFromArray(dilated_array)
...
if __name__ == '__main__':
    ...
    lung_seg_predict_lists = glob.glob(os.path.join(image_dir,'*nii.gz'))
    print(len(lung_seg_predict_lists))
