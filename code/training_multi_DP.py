...
def main(organ,num_epochs,num_classes,datadir,batch_size,save_model_name,segtype,seed=0,amp=True,use_buffer=True,metric_learning=False):
    ...
    total_labels = data_df.iloc[:,-num_classes:].astype(int).values
    ...
    for n,(train_idxs, test_idxs) in enumerate(cv.split(total_images, total_labels, groups)):
        ...
        print(f'number of train images is {num_train_imgs}, number of validation images is {num_val_imgs} label shape {labels_train.shape}')
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(num_train_imgs/batch_size)*num_epochs, eta_min=0.00001)
        ...
        for epoch in range(max_epochs):
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                y_pred = torch.tensor([0, num_classes], dtype=torch.float32, device=device)
                y = torch.tensor([0,num_classes], dtype=torch.long, device=device)
                ...
                print("current epoch: {} current accuracy: {:.4f} current AUC: {:.4f} best AUC: {:.4f} at epoch {} then acc: {:.4f}".format(
                            epoch + 1, acc_metric, auc_result, best_metric, best_metric_epoch, best_acc))
...
if __name__ == '__main__':
    ...
    main(organ,num_epochs,num_classes,datadir,batch_size,save_model_name,segtype,seed,amp=True,use_buffer=True,metric_learning=False)
